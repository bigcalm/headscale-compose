...
Generate the headscale configuration from the template:

```bash
./headscale.sh setup
```

This reads `DOMAIN` and `BASE_DOMAIN` from `.env` and produces `headscale-config/config.yaml`.
`headscale-config/config.yaml` is gitignored to avoid merge conflicts — always use `setup.sh`
...
| `config.tar.gz` | `.env`, `Caddyfile`, `headscale-config/config.yaml`, `headscale-config/acl.hujson`, `headscale-config/extra-records.json` |
...
├── setup.sh                     # Initial setup (generates config.yaml)
├── generate-dns-records.sh      # Extra DNS record generator
├── README.md                    # This file
├── headscale-config/
│   ├── config.yaml              # Headscale configuration (generated by setup.sh)
│   ├── config.yaml.example      # Template for config.yaml generation
