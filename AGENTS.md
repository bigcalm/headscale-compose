...
```bash
./headscale.sh setup         # Generate config.yaml from .env + template
...
- **headscale-config/config.yaml** is generated from `config.yaml.example` by `./setup.sh`. Edit the template or `.env`, then re-run `./headscale.sh setup`.

## Headscale config notes

- `server_url` must be `https://<DOMAIN>` (set via `setup.sh` from `.env`).
- `dns.base_domain` must differ from the `server_url` domain (set in `.env`).
