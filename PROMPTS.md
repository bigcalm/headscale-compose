...
* I backed up my live instance with the backup.sh script. Destroyed the containers, and removed the cloned directory. Cloned a fresh instance. Copied the backups directory into place, and then ran the restore.sh script. It looked promising, but didn't actually restore anything. It silently stopped. Using `echo $?` showed that is failed with exit code `1`. I hadded `-x` to the set options of the restore.sh script. This was the output: [Pasted ~39 lines]
* I copied the updated script to the remote server to test. I then ran the script but intentionally pressed [enter] at the `Restore config files [y/N]` to see what would happen. It skipped the config copy and then failed due to no environment values. I note that the "Service versions" comparision looks odd if there is no exist version to display. It shows an empty string. Can we improve that with something like `[none]`? Output: [Pasted ~25 lines]
* This project wraps existing services. I am not redistributing their work, nor altering it. Can I give my project any licence I like, or should I try to make it align with the services the project wraps? Note that this is all intended to be open source. Also note that the project has been written by AI systems. I, as the project owner, am just the project manager/reviewer/tester. (MIT was chosen)
